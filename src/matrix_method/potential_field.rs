        ...
        for j in 0..col {
            // for z direction
            {
                if i == 0 {
                    ...
                    vf_z[[i, j]] = velocity_potential[[i + 1, j]]
                        - velocity_potential[[i - 1, j]] / (2.0 * (z2 - z1))
                }
            }
            ...
            // for x direction: not used here
            {
                if j == 0 {
                    ...
                    vf_x[[i, j]] = velocity_potential[[i, j + 1]]
                        - velocity_potential[[i, j - 1]] / (2.0 * (x2 - x1))
                }
            }
        }
    }
...
pub fn acoustic_radiation_potential(
    ...
    let velocity_potential = velocity_field(pressure, sp);
    assert_eq!(pressure.shape(), velocity_potential.0.shape());
    ...
    for i in 0..row {
        for j in 0..col {
            let (pmsa, vmsa) = (
                ...
                velocity_potential.0[[i, j]].norm_sqr() + velocity_potential.1[[i, j]].norm_sqr(),
            );
